...
function obtenerUsuario() {
  ...
  do {
    usuario = prompt("ingresá tu nombre");
    console.log(usuario);
...
function renderizarAlbumes(listados) {
...
function mostrarDatosEnPerfil(album) {
  const cantidadAlbumes = document.querySelector("#cant-albums")
  cantidadAlbumes.innerHTML = album.length
  ...
  const cantidadLike = document.querySelector("#cant-favoritos")
  if(likeArray == 1){
    cantidadLike.innerHTML = likeArray + " favorito."
  }else{
    cantidadLike.innerHTML = likeArray + " favoritos."
  }
}
