...
// Hacemos el juego al mejor de 3 partidas👇
do {
    // almacenamos la eleccion del jugador
    const eleccionJugador = pedirJugada();
    // almacenamos la eleccion de la computadora
    const eleccionComputadora = jugadaRandom();

    // calculamos la partida
    const resultadoPartida = compararJugadas(eleccionJugador, eleccionComputadora);
    ...
    if(resultadoPartida.includes('ganaste')){
        ...
        puntajes.jugador++;
    } else if(resultadoPartida === RESULTADOS_POSIBLES[2]){
        // si pierde, le sumamos 1 a la computadora
        puntajes.computadora++;
    }

    
} while (puntajes.jugador < 2 && puntajes.computadora<2);
